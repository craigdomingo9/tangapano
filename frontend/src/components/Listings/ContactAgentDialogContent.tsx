...
function ContactAgentDialogContent() {
  ...
  const [selectedRoom, setSelectedRoom] = useState<Room>(selectedListing.rooms.at(0) as Room);
  ...
  return (
    <div className="bg-white rounded-xl shadow-2xl p-4 w-full max-w-lg relative">
      <p className="text-gray-800 font-semibold text-center text-sm">Please select the room you're interested in.</p>
      <div className="space-y-4 max-h-80 overflow-y-auto pr-2 mt-4">
        {selectedListing.rooms?.map((room, index) => {
          const text = room.gender_preference === "any" 
            ? `${room.max_occupants} student${room.max_occupants > 1 ? "s" : ""}` : 
            `${room.max_occupants} ${room.gender_preference} student${room.max_occupants > 1 ? "s" : ""}`;
          return (
            <div
              key={room.id}
            ...
            >
              <div>
                ...
                <p className="text-gray-600 text-sm">
                  ${parseFloat(room.rent_per_month).toFixed(2)}/month &middot; {text}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="mt-2">
          <Button 
            className="h-12 w-full bg-green-500"
            onClick={handleClick}
            >
              Contact Agent
          </Button>
