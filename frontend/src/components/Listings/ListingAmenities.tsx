  ...
  return (
    <Fragment>
      {matchedAmenities.map((amenity) => (
        <span
          key={amenity.id}
          className="bg-green-100 text-green-700 text-xs font-medium pl-1.5 pr-2.5 py-0.5 rounded-full flex items-center"
        >
      ...
      {missingAmenities.map((amenity) => (
        <span
          key={amenity.id}
          className="bg-red-100 text-red-700 text-xs font-medium pl-1.5 pr-2.5 py-0.5 rounded-full flex items-center"
        >
      ...
      {extraAmenities.map((amenity) => (
        <span
          key={amenity.id}
          className="bg-blue-100 text-blue-700 text-xs font-medium px-2.5 py-0.5 rounded-full flex items-center"
        >
          {amenity.display_name}
        </span>
      ))}
