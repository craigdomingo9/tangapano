...
function ListingImageDialogContent() {
  ...
  const { entities: operation, setEntities: setOperation } = useListingImageDialogOperation();
  ...
  const {
    ...
    queryFn: () => axiosInstance.get(`/listings/listing-images/?listing=${selectedListing?.id}`),
  ...
  const renderRoomList = () => (
    <>
      ...
      {images?.length === 0 ? (
        <p className="text-center text-gray-500 mt-4 text-sm">No images added for this listing yet.</p>
      ) : (
        <div className="space-y-4 max-h-80 overflow-y-auto pr-2 mt-4 grid sm:grid-cols-2">
          {images?.map((image) => (
            <div key={image.id} className="relative group border border-gray-200 rounded-lg overflow-hidden shadow-sm sm:max-w-48">
              <Avatar className="w-full h-32 rounded-none">
                <AvatarImage src={image.image} />
                <AvatarFallback className="rounded-md">{selectedListing.title}</AvatarFallback>
              </Avatar>
              {image.caption && <p className="absolute bottom-0 left-0 right-0 bg-gray-500 text-white text-xs p-1 text-center">{image.caption}</p>}
              <button
