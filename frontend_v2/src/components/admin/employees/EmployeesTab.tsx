    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card/40 backdrop-blur-xl border border-border/40 p-4 rounded-2xl shadow-sm">
                <div className="relative group w-full sm:w-96">
                    ...
                    <input
                        ...
                        className="pl-10 pr-4 h-11 w-full bg-background/50 border border-border/60 rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-lapis/40 transition-all placeholder:text-muted-foreground/60 shadow-inner"
                    />
                </div>
                <button
                    onClick={() => {
                    ...
                    className={`bg-lapis text-white px-6 h-11 rounded-xl text-sm font-bold shadow-lg shadow-lapis/20 transition-all flex items-center gap-2 w-full sm:w-auto justify-center ${hasAddEmployeePermission
            ...
            <div className="md:hidden space-y-4 px-1">
                {employees.map((emp) => (
                    <div key={emp.id} className="bg-card/40 backdrop-blur-xl border border-border/40 rounded-4xl p-6 shadow-sm active:bg-muted/30 transition-all relative overflow-hidden group">
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-lapis/20 to-lapis/5 text-lapis flex items-center justify-center font-bold text-xl border border-lapis/20 shadow-inner">
                                    {(emp.user.first_name || '?').charAt(0)}
                                </div>
                                <div>
                                    <div className="font-bold text-sm">{emp.user.first_name || 'Unknown'} {emp.user.last_name || ''}</div>
            ...
            <div className="hidden md:block bg-card/30 backdrop-blur-2xl border border-border/40 rounded-4xl shadow-xl overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-border/30">
                            {employees.map((emp) => (
                                <tr key={emp.id} className="cursor-default hover:bg-lapis/2 transition-colors group">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="font-bold text-sm tracking-tight">{emp.user.first_name || 'Unknown'} {emp.user.last_name || ''}</div>
