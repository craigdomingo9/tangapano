            ...
            <form id="role-form" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1 space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Policy Identifier</label>
                        <input name="name" type="text" defaultValue={role?.name} required className="w-full bg-muted/20 border border-border/60 rounded-xl px-4 py-3 text-sm font-bold font-bold focus:outline-none focus:ring-2 focus:ring-lapis/40 transition-all" placeholder="Role Label" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Policy Scope</label>
                        <textarea name="description" defaultValue={role?.description || ''} className="w-full h-40 bg-muted/20 border border-border/60 rounded-xl px-4 py-3 text-sm font-bold font-medium focus:outline-none focus:ring-2 focus:ring-lapis/40 transition-all resize-none leading-relaxed" placeholder="Functional scope..." />
                    </div>
                </div>
                <div className="md:col-span-2 space-y-4">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Permission Matrix Engagement</label>
...
const PermissionItem = ({ permission, isChecked, onToggle }: any) => (
    <label className={`flex items-center gap-4 p-4 border rounded-2xl cursor-pointer transition-all duration-300 active:scale-[0.98] ${isChecked ? 'bg-lapis/5 border-lapis/40 shadow-sm' : 'bg-background/40 border-border/40 hover:border-lapis/30'}`}>
        ...
        <div className="flex flex-col min-w-0">
            <span className="text-sm font-bold truncate tracking-tight">{permission.name}</span>
            <span className="text-[9px] text-lapis font-bold uppercase tracking-[0.15em] opacity-60 truncate">{permission.codename}</span>
        </div>
    </label>
);
